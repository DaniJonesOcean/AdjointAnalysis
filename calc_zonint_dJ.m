function [fldOut,X,Y,weightOut]=calc_zonint_dJ(fldIn,method,fldType);
% CALC_ZONINT_DJ(budgIn,method,fldType)
%    computes zonal integral of fldIn (or its fields recursively).
%
...
%    
%    If fldType is 'intensive' then fldIn is mutliplied by 
%    RAC (method=2 or -2) or RAC.*hFacC*DRF (method=1 or -1).
%
% EB additions:
% if fldType is 'dJ' (default), and method=2, then fldIn is multiplied by DXC
...
if isempty(who('fldType')); fldType='dJ'; end;
...
if abs(method)==1;
  ...
  weight=mygrid.mskC(:,:,1).*mygrid.DXC;
...
%   tmp2=nansum(weight(mm,:),1);
    tmp1=nansum(fldIn(mm,:).*weight(mm,:),1);
    tmp2=nansum(weight(mm,:),1);
  end;
